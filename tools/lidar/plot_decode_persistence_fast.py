...
class LidarData:
    def __init__(self, debug=False, persistence_scans=1, packet_batch_size=25, frame_delay=0.0013):
        ...
        self.MAX_DISTANCE = 1000  # in mm (updated)
        self.MIN_DISTANCE = 40    # in mm (updated)
        ...
        self.ax.set_rmax(self.MAX_DISTANCE)  # Updated to 1000 mm
        plt.ion()
    ...
    def plot_data(self):
        ...
        angles, distances = [], []
        for angle_rad, (dist, _) in self.data_buffer.items():
        ...
        if angles:
            self.ax.clear()
            self.ax.scatter(angles, distances, c='blue', s=10)
            self.ax.set_rmax(self.MAX_DISTANCE)
            plt.draw()
            plt.pause(0.001)
            self._print(f"Plotted {len(angles)} points, angles from {min(angles)*180/math.pi:.1f}° to {max(angles)*180/math.pi:.1f}°")
        else:
    ...
    def process_stdin(self):
        ...
        if elapsed_time > 0:
            ...
            print(f"Processed {self.total_packet_count} packets in {elapsed_time:.2f} seconds ({fps:.2f} FPS)")
...
if __name__ == '__main__':
    ...
    try:
        sensor = LidarData(debug=args.debug, persistence_scans=2, packet_batch_size=args.batch_size, frame_delay=0.0013)
